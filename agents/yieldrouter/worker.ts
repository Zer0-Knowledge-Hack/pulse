/**
 * Public Cloudflare Worker for yieldrouter agent.
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/ping" || url.pathname === "/") {
      return jsonResponse({
        status: "HEALTHY",
        agent: "yieldrouter-agent",
        version: "0.1.0",
        network: "bsc-testnet",
        chainId: 97,
        wallet: "0xe1D07be03DDE2C292f842AdE4f34782FDf9176c5",
        endpoints: {
    ...
    if (url.pathname === "/.well-known/agent-card.json") {
      const agentCard = {
        ...
        description: "ERC-8183 seller agent (yieldrouter-agent) — yield optimization and APR comparisons across BNB Chain vaults and money markets.",
        url: `${hostUrl}/`,
        version: "1.0.0",
        ...
        skills: [
          ...
          },
          {
            ...
            description:
              'After funding on-chain, send {"skill": "notify_funded", "job_id": <int>}. The seller executes yield strategy comparisons and routing analysis.',
                ...
                tools: [
                  {
                    ...
                    description: "Fetch and rank highest risk-adjusted yield opportunities across Venus, PancakeSwap, and Beefy on BNB Chain.",
                    inputSchema: {
                      type: "object",
                      properties: {
                        asset: { type: "string", description: "Asset symbol (e.g. BNB, USDT, USDC)" },
                        min_tvl_usd: { type: "number", description: "Minimum pool TVL filter in USD" },
                      },
                      required: ["asset"],
                    },
                  },
