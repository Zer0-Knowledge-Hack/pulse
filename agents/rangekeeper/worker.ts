/**
 * Public Cloudflare Worker for rangekeeper agent.
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/ping" || url.pathname === "/") {
      return jsonResponse({
        status: "HEALTHY",
        agent: "rangekeeper-agent",
        version: "0.1.0",
        network: "bsc-testnet",
        chainId: 97,
        wallet: "0xCC2abE29F43EAb530a6b5D93E3C41bc0E7622b47",
        endpoints: {
    ...
    if (url.pathname === "/.well-known/agent-card.json") {
      const agentCard = {
        ...
        description: "ERC-8183 seller agent (rangekeeper-agent) — liquidity range calculation & rebalancing on PancakeSwap v3 (BNB Chain).",
        url: `${hostUrl}/`,
        version: "1.0.0",
        ...
        skills: [
          ...
          },
          {
            ...
            description:
              'After funding on-chain, send {"skill": "notify_funded", "job_id": <int>}. The seller executes liquidity range rebalancing calculation.',
                ...
                tools: [
                  {
                    ...
                    description: "Calculate optimal liquidity price range and rebalancing thresholds for PancakeSwap v3 pools.",
                    inputSchema: {
                      type: "object",
                      properties: {
                        pool: { type: "string", description: "PancakeSwap v3 pool address or pair symbol" },
                        tolerance_pct: { type: "number", description: "Price drift tolerance percentage before rebalancing" },
                      },
                      required: ["pool"],
                    },
                  },
